...
class Interpolator:
    ...
    def show_interpolation_point(self):
        ...
        if self.ax is None: self.ax = plot.axes(projection="3d")
        self.__plot_point(self.input_point, 'black', 'o')
        plot.show(block=False)
        plot.pause(0.001)
    ...
    def get_interpolated_point(self):
        if self.shape == Interpolator.SURFACE:
            self.output_point = Point(self.input_point.x, self.input_point.y, self.surface(self.input_point.x, self.input_point.y).tolist())
